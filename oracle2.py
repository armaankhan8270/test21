...
ORDER BY COLUMN_ID
"""
        # Create temporary files for spooling output and the SQL script.
...
""")
        ...
        try:
            ...
            column_data = [(col.strip(), dt.strip()) for col, dt in data if len(col) > 0 and len(dt) > 0]
            ...
            with out_fp as f_out:
                ...
                while True:
                    ...
                    with temp_raw.open("r", encoding="utf-8", errors="replace") as f_in:
                        # Read all rows at once and write them.
